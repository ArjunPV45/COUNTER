...
Contains all HTTP route handlers for the web application.
"""
...
def register_routes(app: Flask, user_data, pipeline_manager, video_stream_manager: VideoStreamManager):
    ...
    def start_pipeline():
        ...
        try:
            ...
            if success:
                return jsonify({
                    "success": True,
                    "message": "Pipeline started with validated sources"
                }), 200
            else:
                return jsonify({
                    "success": False,
                    "message": "Failed to start pipeline - check RTSP sources"
                }), 400
    ...
    def stop_pipeline():
        ...
        try:
            ...
            return jsonify({
                "success": False,
                "message": f"Error stopping pipeline: {e}"
            }), 500
    ...
    def create_camera_zone(camera_id):
        """Create a new zone for a specific camera."""
        data = request.json
        if not data:
        ...
        if success:
            ...
            print(f"[HTTP API] Zone updated - Camera: {camera_id}, Zone: {zone}")
            print(f"[HTTP API] Zone coordinates: top_left={top_left}, bottom_right={bottom_right}")
            print(f"[HTTP API] Full zone data: {zone_data}")
            print(f"[HTTP API] All zones for camera {camera_id}: {user_data.data[camera_id]['zones']}")
            ...
            print(f"[HTTP API] Failed to create/update zone {zone} for camera {camera_id}")
